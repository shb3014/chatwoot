module Captain::ChatHelper
  ...
  def request_chat_completion
    ...
    if is_deepseek_v32
      ...
      parameters[:enable_thinking] = true if is_qwen
      captain_logger.warn 'Thinking mode enabled - response format constraint removed, relying on prompt for JSON' if has_tools
    elsif is_qwen
      # Qwen3+ models have built-in reasoning enabled by default; disable to avoid massive latency.
      parameters[:enable_thinking] = false
      captain_logger.info 'Qwen model: enable_thinking=false'
      captain_logger.info 'Qwen model with tools: response_format disabled' if has_tools
    end
    ...
    tool_calls_array = tool_calls.keys.sort.map { |index| tool_calls[index] }
    {
      'choices' => [{
        'message' => {
          ...
          'reasoning_content' => reasoning.presence,
  ...
  def handle_response(response)
    ...
    if reasoning_content.present?
      captain_logger.info "[Response] reasoning_content present: #{reasoning_content.length} chars"
      captain_logger.debug "Reasoning content: #{reasoning_content}"
    end
    ...
    if message['tool_calls']
      ...
      if content.empty?
        captain_logger.warn 'LLM returned empty content with no tool calls, using fallback response'
        fallback_message = {
      ...
      begin
        ...
        if @response_validator
          ...
          captain_logger.info "[Validation Fallback] valid=#{validation[:valid]} reject=#{validation[:should_reject]} reason=\"#{validation[:reason]}\""
  ...
  def attempt_validation_retry(original_content, validation)
    return nil if @validation_retry_attempted

    @validation_retry_attempted = true
    captain_logger.info 'Attempting retry with corrective guidance after validation rejection'

    @messages << { role: 'assistant', content: original_content }
    @messages << {
      role: 'system',
      content: "CRITICAL: Your previous response was REJECTED by our quality system because: #{validation[:reason]}. " \
               "You MUST rewrite your response following these rules:\n" \
               "1. Use ONLY information that appears in the search results provided to you.\n" \
               "2. Do NOT include any numbers, statistics, or timeframes not explicitly stated in the search results.\n" \
               "3. Do NOT use hedging phrases like 'typically', 'usually', 'generally', or 'in most cases'.\n" \
               '4. If the search results do not contain the specific information the user asked about, state clearly that ' \
               "you don't have that information and offer to connect them with a human support agent.\n" \
               "5. Output valid JSON with 'reasoning' and 'response' fields."
    }

    saved_callback = @streaming_callback
  ...
  def parse_referenced_articles_from_text(text)
    ...
    if text.match?(/\*\*(?:Sources|Referenced Articles)\*\*:?/i)
      ...
      section.to_s.lines.each do |line|
        ...
        internal_match = stripped.match(/^\[\d+\]\s+(.+?)\s+\(type:\s*([^)]+)\)\s*$/)
        next unless internal_match

        references << {
          title: internal_match[1].strip,
          locale: nil,
          url: '',
          type: internal_match[2].strip
        }
      end
    end
  ...
  def append_reference_list(content, references)
    ...
    visible_refs = []
    renumber_map = {}
    normalized_references.each_with_index do |ref, idx|
      old_num = idx + 1
      if ref[:type] == 'internal'
        renumber_map[old_num] = nil
      else
        visible_refs << ref
        renumber_map[old_num] = visible_refs.size
      end
    end

    processed_content = processed_content.gsub(/\s*\[(\d+)\]/) do
      ref_num = ::Regexp.last_match(1).to_i
      new_num = renumber_map[ref_num]
      new_num ? "[#{new_num}]" : ''
    end

    processed_content = convert_inline_citations(processed_content, visible_refs)

    return processed_content.rstrip if visible_refs.empty?

    citation_chips = visible_refs.map.with_index do |reference, index|
      ref_num = index + 1
      title_escaped = CGI.escapeHTML(reference[:title].to_s)
      url_escaped = CGI.escapeHTML(reference[:url].to_s)
      ref_type = CGI.escapeHTML(reference[:type] || 'article')
  ...
  def should_force_search?
    return false if @skip_forced_search # prevent infinite loop
    ...
    when 'continuation', 'new_question'
      captain_logger.info "LLM classified as #{classification} - forcing search"
      classification
    else
  ...
  # Priority: CAPTAIN_CLASSIFICATION_MODEL > CAPTAIN_FAST_MODEL > current model
