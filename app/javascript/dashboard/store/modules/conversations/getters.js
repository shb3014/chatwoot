  ...
  getUnreadChats: _state => activeFilters => {
    return _state.allConversations
      .filter(conversation => {
        ...
        const shouldFilter = applyPageFilters(conversation, activeFilters);
        return hasUnread && shouldFilter;
      })
      .sort((a, b) => sortComparator(a, b, _state.chatSortFilter));
  },
  getUnresolvedChats: (_state, _, __, rootGetters) => activeFilters => {
    const currentUser = rootGetters.getCurrentUser;
    const currentUserId = rootGetters.getCurrentUser.id;
    const currentAccountId = rootGetters.getCurrentAccountId;
    ...
    return _state.allConversations
      .filter(conversation => {
        const isUnresolved = conversation.status !== 'resolved';
        const shouldFilter = applyPageFilters(conversation, activeFilters);
        const allowedForRole = applyRoleFilter(
          conversation,
          userRole,
        ...
        return isUnresolved && shouldFilter && allowedForRole;
      })
      .sort((a, b) => sortComparator(a, b, _state.chatSortFilter));
  },
